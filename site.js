const translations = {
  zh: {
    ...
    description: "SleeePal 以 Mac 桌面版为首发路径，把本地床头近场试验、三轨可视化与夜报带到枕边；蓝牙可先试用，有线是当前更可信的验证路径。",
    "nav.philosophy": "理念",
    "nav.science": "原理",
    "nav.features": "功能",
    "nav.visual": "可视化",
    "nav.cta": "立即体验",
    "hero.pretitle": "Mac-first 首发 · 床头近场试验 · sleeepal.com",
    "hero.kicker": "您从未拥有过",
    ...
    "hero.primary": "查看公开进展",
    "hero.secondary": "了解床头近场试验",
    "hero.wave.antiMeta": "延迟 12ms · −2 dB",
    "marquee.1": "44,100 SAMPLES PER SECOND",
    "marquee.2": "床头近场试验 · 先试用再验证",
    "marquee.3": "QUIETER NIGHTS, CAREFULLY TESTED",
    ...
    "brand.sub": "主动降噪耳机把控制点放进耳道，SleeePal 则尝试把这套物理思路带到枕边近场试验。<br/>当前首发以 Mac 桌面版为主，iPhone 负责 setup 与报告 companion。",
    "philosophy.quote": "良好的睡眠，胜过任何奢侈品——不是吗？",
    "science.kicker": "床头近场试验 · 技术路径",
    "science.title": "本地声学验证，<br/><em>从耳道思路走向床头近场试验</em>",
    "science.desc": "主动降噪耳机把控制点放进耳道。<br/>SleeePal 借鉴相近的物理思路，把首发重点放在 <strong>Mac + 外接音箱</strong> 的床头近场试验与链路验证，而不是先承诺固定的安静结果。",
    ...
    "science.compare.expand": "转向床头近场试验",
    "science.compare.launch.zone": "验证区域 = 枕边近场",
    "science.compare.launch.body": "麦克风在 Mac / 手机里<br/>验证区域在<strong>枕边近场</strong><br/>蓝牙先试用，有线更可信",
    "science.spec1": "Hz 采样率，实时处理每一帧声音",
    "science.spec2": "样本/帧，<23ms 延迟窗口",
    "science.spec3": "Hz 鼾声频段精准侦测",
    "science.spec4": "路实时波形，效果可见可验证",
    "science.diagram.zoneTitle": "验证中的近场区域",
    ...
    "science.step4.title": "床头近场验证",
    "science.step4.body": "在小型有线音箱链路上，持续验证枕边近场是否真的出现可复现改善。三路实时波形与夜报会帮助你判断效果，而不是只靠主观期待。",
    "science.why.kicker": "为什么近场有效",
    "science.why.title": "不追求全屋静音，<br/><em>只验证床头附近是否能更柔和</em>",
    "science.why.body1": "全屋级主动降噪需要专业声学工程和大量设备，成本极高且实现困难。SleeePal 选择更务实的路径：先观察头部附近的小范围是否出现可复现的柔化，再把证据留给夜报和复现测试。",
    "science.why.body2": "这与主动降噪耳机在耳道里建立控制点的思路相近，只是首发版本更克制：蓝牙先建立体验，Mac + 有线链路才是当前更可信的验证路径。",
    ...
    "faq.kicker": "常见问题",
    "faq.title": "关于床头近场试验、音箱和白噪声推荐",
    "faq.q1": "音箱体积越大，改善效果就越好吗？",
    "faq.a1": "不一定。近场宁静更依赖低延迟、稳定摆位、音箱离头部足够近、麦克风位置固定。更大的音箱通常有更好的低频余量，适合空调、交通、低频嗡声；但如果离得远、音量过大或延迟高，反而会让房间反射和回授更难控制。",
    "faq.q2": "音量应该开多大？",
    "faq.a2": "音量不是越大越好。理想状态是刚好足够形成近场反相或舒适遮蔽，又不会让麦克风主要听到软件自己播出的声音。软件会用电平、估算变化和安全门控提示工作区，用户应从小音量开始，逐步调到“变化可见但不吵”的位置。",
    "faq.q3": "能根据环境噪音自动推荐白噪声吗？",
    "faq.a3": "可以，这是产品应该具备的方向。系统先分析环境频谱和突发特征：低频轰鸣优先推荐棕噪、风扇或海浪；宽频嘶声可推荐粉噪、雨声或溪流；断续人声和邻居噪声更适合雨声、森林或流动水声。算法给出候选，最终由用户按舒适度选择。",
    "faq.q4": "白噪声会不会被软件误当成环境噪音继续反相？",
    "faq.a4": "设计目标是不会。软件知道自己正在播放哪一种声景，会为每种声景建立排除模型，把这部分从麦克风输入和反相参考里剔除。这样反相输出主要追踪真实环境噪音，而不是追踪软件自己播出的助眠背景声。",
    "faq.q5": "这是真正的主动降噪，还是白噪声遮蔽？",
    "faq.a5": "两者都有，但需要分清。主动反相更适合持续、可预测、低频到中低频的噪声；白噪声更适合提高舒适感、遮盖断续声和帮助入睡。SleeePal 的目标是把反相、助眠声景和夜间证据放在同一个闭环里，而不是只做一个播放器。",
    "faq.q6": "为什么有线链路比蓝牙更推荐？",
    "faq.a6": "主动反相对延迟非常敏感。蓝牙适合先体验助眠声景和基础监测，但延迟和系统缓冲不稳定，会降低反相抵消的可控性。想认真验证床头近场试验，建议使用有线音箱或低延迟外置声卡。",
    "faq.q7": "为什么改善值不是 100%？",
    "faq.a7": "床头不是耳机耳道，声音会被墙面、床品、人体和音箱位置反射。软件显示的是近场估算与趋势，真实体感还会受摆位影响。稳定降低一部分干扰、让声音更柔和，通常比承诺完全消音更可信。",
    "faq.q8": "麦克风录到的声音会上传吗？",
    "faq.a8": "当前产品思路是优先本地处理：实时分析、反相、声景排除和夜间事件记录都应尽量在本机完成。若未来加入云端报告或同步，也应该明确开关、用途和保存范围，让用户知道哪些数据离开了设备。",
    "who.kicker": "守护对象",
    ...
    "who.card4.solution": "这类场景更适合先做 softening、masking 和场景仪式，不把它包装成已验证的固定安静结果。",
    ...
    "features.1.title": "床头链路守护",
    "features.1.body": "首发版本的核心不是盲目夸大强降噪，而是把本地监测、场景引导、路由适配输出和有线校准做扎实。蓝牙可先试用建立信任，有线才是当前更可信的验证路径。",
    ...
    "testimony.3.body": "午休与短时小憩更适合手机单机或蓝牙试用路径。这里更现实的价值，是快速进入状态、叠加声景与轻度 softening，而不是宣称已经形成固定安静结果。",
  ...
  en: {
    ...
    description: "SleeePal is launching Mac-first: local bedside near-field trials, three-track visuals, and night reports now; bluetooth can be the trial path, while wired is the current more credible validation path.",
    "nav.philosophy": "Philosophy",
    "nav.science": "Science",
    "nav.features": "Features",
    "nav.visual": "Visuals",
    "nav.cta": "Try it",
    "hero.pretitle": "Mac-first launch · bedside near-field trial · sleeepal.com",
    ...
    "marquee.1": "44,100 SAMPLES PER SECOND",
    "marquee.2": "BEDSIDE NEAR-FIELD TRIAL · TRY BEFORE VALIDATION",
    "marquee.3": "QUIETER NIGHTS, CAREFULLY TESTED",
    ...
    "brand.sub": "Noise-cancelling headphones place the control point inside the ear canal. SleeePal is trying to bring a related physical idea into bedside near-field trials.<br/>The current launch is Mac-first, while iPhone handles setup and report companion work.",
    "philosophy.quote": "Good sleep beats every luxury item, doesn't it?",
    "science.kicker": "Bedside near-field trial · How it works",
    "science.title": "Local acoustic validation,<br/><em>from ear-canal logic to bedside near-field trials</em>",
    "science.desc": "ANC headphones place the control point inside the ear. SleeePal borrows related physical logic and focuses the first release on <strong>Mac + external speaker</strong> bedside trials, not on promising a fixed quiet outcome upfront.",
    ...
    "science.compare.expand": "Shifted into bedside near-field trials",
    "science.compare.launch.zone": "Validation area = bedside near field",
    "science.compare.launch.body": "The microphone lives in the Mac or phone<br/>The validation area lives in the <strong>bedside near field</strong><br/>Bluetooth is the trial path, wired is more credible",
    ...
    "science.diagram.zoneTitle": "Near-field area under validation",
    ...
    "science.step4.title": "Bedside validation",
    ...
    "science.why.title": "Not trying to quiet the whole room,<br/><em>only testing whether the bedside area can feel softer</em>",
    "science.why.body1": "Whole-room active noise control needs serious acoustic engineering and a lot of hardware. SleeePal takes the more practical path: observe whether a small area near the head becomes softer in a repeatable way, then keep the evidence in reports.",
    "science.why.body2": "It follows logic related to ANC headphones creating a control point in the ear canal, but the launch version is more restrained: bluetooth builds the first experience, while Mac plus wired is the more credible validation path today.",
    ...
    "faq.kicker": "FAQ",
    "faq.title": "Bedside near-field trials, speakers, and noise-sound recommendations",
    "faq.q1": "Does a bigger speaker always improve the result?",
    "faq.a1": "Not always. Near-field quieting depends more on low latency, stable placement, short distance to the head, and a fixed mic position. Bigger speakers usually have more low-frequency headroom for HVAC, traffic, and hum, but distance, excessive volume, or high latency can make reflections and feedback harder to control.",
    "faq.q2": "How loud should the speaker be?",
    "faq.a2": "Louder is not automatically better. The target is enough level for near-field anti-phase or comfortable masking, without the mic mostly hearing the app's own sound. SleeePal uses level, estimated change, and safety gating to show the working zone. Start low and raise it only until the room feels softer without becoming noisy.",
    "faq.q3": "Can SleeePal recommend a soundscape based on room noise?",
    "faq.a3": "Yes, that is the right product direction. The system can analyse spectrum and event patterns: brown noise, fan, or ocean for low rumble; pink noise, rain, or stream for broadband hiss; rain, forest, or moving water for intermittent speech and neighbour noise. The algorithm recommends candidates, while the user chooses the most comfortable one.",
    "faq.q4": "Will the app's white noise be mistaken as room noise?",
    "faq.a4": "The design goal is no. SleeePal knows which soundscape it is playing and builds an exclusion model for each mode, removing that app-generated sound from the mic reference and anti-phase target. Anti-phase should follow real room noise, not the app's own sleep sound.",
    "faq.q5": "Is this true active noise cancellation or white-noise masking?",
    "faq.a5": "It uses both, but they should not be confused. Active anti-phase is best for steady, predictable low to mid-low noise. Soundscapes help comfort, sleep onset, and intermittent noises. SleeePal's goal is to combine anti-phase, soundscapes, and night evidence in one loop, not to be only a player.",
    "faq.q6": "Why is wired audio recommended over bluetooth?",
    "faq.a6": "Active anti-phase is latency-sensitive. Bluetooth is good for trying soundscapes and basic monitoring, but its delay and buffering are less stable. For serious bedside near-field validation, use a wired speaker or low-latency external audio interface.",
    "faq.q7": "Why is improvement not 100%?",
    "faq.a7": "A bedside area is not an ear canal. Walls, bedding, body position, and speaker placement all reflect sound. The app shows near-field estimates and trends; perceived results still depend on geometry. A stable partial reduction and softer noise are more credible than promising total silence.",
    "faq.q8": "Does microphone audio get uploaded?",
    "faq.a8": "The product direction is local-first: realtime analysis, anti-phase, soundscape exclusion, and night events should run on the device whenever possible. If cloud reports or sync are added later, they should have explicit controls and clear explanations of what leaves the device.",
    ...
    "who.card4.solution": "This is better treated as a softening, masking, and scenario ritual path first, not as proof of a fixed quiet outcome.",
    ...
    "features.1.title": "Bedside route guard",
    "features.1.body": "The launch is not about pretending to have maximum ANC already. It is about making local monitoring, scenario guidance, route-aware output, and wired calibration credible. Bluetooth is the trial path; wired is the more credible validation path today.",
    ...
    "testimony.3.body": "For power naps, phone-only or bluetooth trial routes are the more realistic fit. The value there is faster settling, layered soundscapes, and light softening, not a claim of a fixed quiet outcome.",
